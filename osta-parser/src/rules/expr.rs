...
pub fn parse_term(
    ...
    let node = match token.kind {
        ...
        TokenKind::Minus | TokenKind::Bang => {
            tokenizer.next();
            // FIX(JohanVonElectrum): -x + y should be parsed as (-x) + y, not -(x + y)
            let expr = parse_expression(tokenizer, builder)?;
